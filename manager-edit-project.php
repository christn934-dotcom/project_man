...
$manager_id_session = (int) $_SESSION["user_id"];
...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    ...
    /* Verify this manager owns the project */
    $stmt = mysqli_prepare($conn, "SELECT * FROM projects WHERE id = ? AND manager_id = ?");
    mysqli_stmt_bind_param($stmt, "ii", $project_id, $manager_id_session);
    ...
    if (!$project) {
        header("Location: manager-projects.php?error=" . urlencode("Project not found or access denied."));
        exit;
    }
    ...
    if (!empty($assigned_member_ids)) {
        $placeholders = implode(",", array_fill(0, count($assigned_member_ids), "?"));
        $types = str_repeat("i", count($assigned_member_ids));
        $in_stmt = mysqli_prepare($conn, "SELECT id, full_name, email, profile_image FROM users WHERE id IN ($placeholders) ORDER BY full_name ASC");
        mysqli_stmt_bind_param($in_stmt, $types, ...$assigned_member_ids);
        mysqli_stmt_execute($in_stmt);
        $in_result = mysqli_stmt_get_result($in_stmt);
        if ($in_result) {
            while ($row = mysqli_fetch_assoc($in_result)) {
                $assigned_members[] = $row;
            }
        }
...
<div class="admin-layout">
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a href="manager-dashboard.php" class="nav-item"><span class="nav-icon">▦</span> Dashboard</a>
            <a href="manager-projects.php" class="nav-item active"><span class="nav-icon">▣</span> Projects</a>
            <a href="manager-tasks.php" class="nav-item"><span class="nav-icon">✓</span> Tasks</a>
            <p class="nav-title">TEAM</p>
            <a href="manager-team.php" class="nav-item"><span class="nav-icon">♙</span> Team</a>
            <a href="manager-reports.php" class="nav-item"><span class="nav-icon">▥</span> Reports</a>
            <a href="notifications.php" class="nav-item"><span class="nav-icon">♧</span> Notifications</a>
            <p class="nav-title">ACCOUNT</p>
            <a href="manager-settings.php" class="nav-item"><span class="nav-icon">⚙</span> Settings</a>
        ...
        <header class="topbar">
            ...
            <div class="topbar-right">
                ...
                <div class="admin-profile">
                    ...
                    <div class="profile-info"><strong><?= htmlspecialchars($manager_name) ?></strong><span>Project Manager</span></div>
        ...
        <section class="dashboard-content">
            <div class="page-header">
                <div>
                    ...
                    <h1>Edit Project</h1>
                    <p>Update project details and status.</p>
                </div>
                <div class="page-actions">
                    <a href="manager-projects.php" class="secondary-button">← Back to Projects</a>
                </div>
            </div>
            ...
            <div class="form-container">
                <div class="dashboard-card">
                    <form method="POST" action="manager-edit-project.php">
                        ...
                        <div class="form-row">
                            <div class="form-group">
                                ...
                                <select name="status">
                                    ...
                                    <option value="pending_approval" <?= ($project["status"] === "pending_approval") ? "selected" : "" ?>>⏳ Submit for Approval</option>
                        ...
                        <div class="modal-actions">
                            <a href="manager-projects.php" class="secondary-button">Cancel</a>
...
/* Verify ownership */
$check = mysqli_prepare($conn, "SELECT id FROM projects WHERE id = ? AND manager_id = ?");
mysqli_stmt_bind_param($check, "ii", $project_id, $manager_id_session);
mysqli_stmt_execute($check);
$check_result = mysqli_stmt_get_result($check);
if (mysqli_num_rows($check_result) === 0) {
    mysqli_stmt_close($check);
    header("Location: manager-projects.php?error=" . urlencode("Access denied."));
    exit;
}
mysqli_stmt_close($check);
...
} elseif (!in_array($status, ["planning", "in_progress", "on_hold", "pending_approval", "cancelled"], true)) {
    $error = "Invalid status. Managers cannot directly set a project to Completed — submit for approval instead.";
...
if ($error === "" && $project_id > 0) {
    ...
    $valid_transitions = [
        ...
        "pending_approval" => ["in_progress", "cancelled"],  /* manager can withdraw */
        "completed"       => [],
        "cancelled"       => [],
    ];
...
if ($error === "") {
    ...
    try {
        ...
        $update = mysqli_prepare($conn, "UPDATE projects SET name = ?, description = ?, start_date = ?, end_date = NULLIF(?, ''), status = ?, priority = ? WHERE id = ?");
        mysqli_stmt_bind_param($update, "ssssssi", $name, $description, $start_date, $end_date, $status, $priority, $project_id);
        ...
        /* Activity Log — special action if submitting for approval */
        if ($status === "pending_approval") {
            $action = "project_submitted_for_approval";
            $log_description = "Manager submitted project \"" . $name . "\" for admin approval";
        } else {
            $action = "project_updated";
            $log_description = "Manager updated project: " . $name;
        }

        $activity = mysqli_prepare($conn, "INSERT INTO activity_logs (user_id, project_id, action, description) VALUES (?, ?, ?, ?)");
        mysqli_stmt_bind_param($activity, "iiss", $manager_id_session, $project_id, $action, $log_description);
        if (!mysqli_stmt_execute($activity)) {
            throw new Exception("Failed to log activity.");
        }
        mysqli_stmt_close($activity);

        /* Email notification — notify admin + members */
        send_notification_email($conn, $action, $log_description, $project_id, $manager_id_session);

        mysqli_commit($conn);
        header("Location: manager-projects.php?updated=1");
        exit;
...
header("Location: manager-edit-project.php?id=" . $project_id . "&error=" . urlencode($error));
exit;
?>
